...
class OdooInvoice(OdooObject, SqliteObject):
    def __init__(self, odoo_info, id):
        logger.debug("Init Invoice: %i", id)
        super().__init__(odoo_info, id) 
        self.invoice = self.get_from_odoo()
        if type(self.invoice) == list:
    ...
    def get_from_odoo(self):
        ...
        if cr != False:
            return cr 
        try:
            return self.odoo_info.kw_read_result('account.invoice', [self.id])
    ...
    def write_to_database(self, odoo_info):
        ...
        domain = [field_list] 
        # print(domain)
        result = odoo_info.kw_create('account.invoice', domain)
    ...
    def delete_from_database(self, odoo_info):
        ...
        domain = [[self.id]]
        print(domain)
        result = odoo_info.kw_delete('account.invoice', domain)
    ...
    def write_info_to_csv(self, f):
        ...
        for i_key in [
            ...
            if i_key in ['invoice_id', 'parent_id']:
